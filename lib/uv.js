...
uv.stop = function(loop) {
    return uvb.stop(loop);
};
...
uv.stream_new = function(stream, backlog, cb) {
    return uvb.__stream_new();
};

uv.listen = function(stream, backlog, cb) {
    return uvb.listen(stream, backlog, cb);
};

uv.tcp_init = function(loop, stream) {
    return uvb.tcp_init(loop, stream);
};

uv.close = function(handle, cb) {
    return uvb.close(handle, cb);
};
